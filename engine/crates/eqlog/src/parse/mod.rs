...
pub struct Parser {
    ...
    /// The last stamp text and its instant (Z Engine, 2026-09-06). A log is bursts of lines inside
    /// one second, so the previous line's stamp answers most lines without the stamp regex, the date
    /// arithmetic or the zone conversion. Exact: the same text in the same zone is the same instant.
    stamp_memo: std::cell::RefCell<(String, i64)>,
}
...
impl Parser {
    ...
    pub fn new(clock: Clock, db: Option<Arc<SpellDb>>, character: Option<String>) -> Self {
        Parser {
            ...
            quest: quest::QuestRes::new(),
            unparsed_stamps: std::cell::Cell::new(0),
            stamp_memo: std::cell::RefCell::new((String::new(), 0)),
        }
    }
    ...
    /// The stamp's instant, through the memo above. An unparsable stamp is never remembered, so it
    /// is counted every time it appears, exactly as it was before the memo.
    fn stamp_ts(&self, stamp: &str) -> i64 {
        {
            let memo = self.stamp_memo.borrow();
            if memo.0 == stamp {
                return memo.1;
            }
        }
        let ts = self.clock.parse_eq_timestamp(stamp);
        if ts == 0 {
            self.unparsed_stamps.set(self.unparsed_stamps.get() + 1);
        } else {
            let mut memo = self.stamp_memo.borrow_mut();
            memo.0.clear();
            memo.0.push_str(stamp);
            memo.1 = ts;
        }
        ts
    }
    ...
    fn classify(&self, c: &Ctx, out: &mut Ev) {
        ...
        let claimed = combat::classify_miss(&self.combat, c, out)
            ...
            || quest::classify_hail(&self.quest, c, out)
            ...
            || quest::classify_faction_hit(&self.quest, c, out)
