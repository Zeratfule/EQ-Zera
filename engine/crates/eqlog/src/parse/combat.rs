...
mod verbs;
pub use verbs::{melee_skill, melee_verb_base};
...
fn taken_damage(r: &CombatRes, c: &Ctx, out: &mut Ev) -> bool {
    // The first-person line (`You have taken 12 damage from Choking by a thunder spirit princess.`)
    // is the third-person shape with a different verb (Z Engine, 2026-09-07: ~1,500 of the owner's
    // lines, every one `unknown`). Respelled, it takes the same road and the same fields, with the
    // target `You`; the raw line in the envelope is the one the game printed.
    let respelled;
    let text = if c.text.starts_with("You have taken ") {
        respelled = c.text.replacen("You have taken ", "You has taken ", 1);
        respelled.as_str()
    } else {
        c.text
    };
...
pub fn classify_damage(r: &CombatRes, c: &Ctx, out: &mut Ev) -> bool {
    ...
    let has_taken = c.text.contains("has taken") || c.text.starts_with("You have taken ");
