...
///
/// The loot family's sold clause reads through it too (see [`sold_price`] below), so a shape one of
/// them learns cannot be a shape the other mis-reads.
...
/// The price a sold LOOT line stated, appended to the row `world.rs`'s shared `loot` helper has
/// already written.
///
/// It lives here, with the other coin-clause readers, because that clause is the same grammar in
/// the same words — the sentence is a loot line, but the money in it is not a different fact from
/// the money in `You receive …`.
///
/// APPENDED LAST, so every byte a sold event printed before this existed is still the same byte:
/// the key arrives after `count`, which is where `combined`'s `created` also goes.
///
/// `for free.` is `{}` — present and empty, the shape `purchase`'s price already uses for the free
/// merchant form. It is the honest reading: the sentence DID state a price, and the price was
/// nothing. An unreadable clause writes no key at all, which is the other honest answer.
pub fn sold_price(r: &AcquireRes, out: &mut Ev, clause: &str) {
    if clause == "free" {
        out.coins(Key::Coins, &[]);
        return;
    }
    if let Some(coins) = parse_coins(r, clause) {
