...
pub fn scan_bytes(parser: &Parser, bytes: &[u8], emit: impl FnMut(&str, &Payload)) -> u64 {
    scan_bytes_from(parser, bytes, 0, emit)
}

/// [`scan_bytes`] with the first event numbered `first_seq` - the continuation of a fold that
/// resumed from a checkpoint (Z Engine): the events after the cut carry the sequence numbers a
/// whole scan would have given them, so `seq`-valued state agrees byte for byte.
pub fn scan_bytes_from(
    parser: &Parser,
    bytes: &[u8],
    first_seq: i64,
    mut emit: impl FnMut(&str, &Payload),
) -> u64 {
