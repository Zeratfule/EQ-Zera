...
1. The supervisor spawns `zengine.exe` with **no secrets in argv or env**. The **first line on
...
2. The engine binds `127.0.0.1:0` and prints **exactly one line** to stdout, flushed:
   ...
   diagnostics go to stderr, tagged `[z-engine]`.
...
# staged C:\Users\…\Temp\zengine-482-E3FlHx\eqlog_Primitive_freeport.txt (9185395 bytes)
...
[z-engine] ingest: spell db ready in 398 ms
...
[z-engine] fold landed: 139862 events, mark 9185395 of C:\Users\…\eqlog_Primitive_freeport.txt, now live
...
# staged C:\Users\…\Temp\zengine-480-fJUtbz\eqlog_Primitive_freeport.txt (9185598 bytes)
...
[z-engine] ingest: spell db ready in 383 ms
...
[z-engine] fold landed: 139864 events, mark 9185598 of C:\Users\…\eqlog_Primitive_freeport.txt, now live
[z-engine] views: loot.ledger 1 frames (1 reset / 0 diff), 3 rows, 0 ops, 593 B (widest 593 B); fold->frame mean 29 us max 29 us over 1
...
[z-engine] views: loot.ledger 2 frames (1 reset / 1 diff), 3 rows, 2 ops, 892 B (widest 593 B); fold->frame mean 50.3 ms max 100.6 ms over 2
```
...
# staged C:\Users\…\Temp\zengine-485-w75M8Y\eqlog_Primitive_freeport.txt (9185639 bytes)
...
-> {"id":3,"op":"combat.snapshot","params":{}}
<- {"error":{"code":"unavailable","message":"no log is attached, so there is no fold to ask"},"id":3,"kind":"error","ok":false}
[z-engine] ingest: spell db ready in 400 ms
-> {"id":3,"op":"combat.snapshot","params":{}}
...
<- {"epoch":2,"kind":"epoch","progress":{"events":139865,"pct":100.0},"reason":"progress"}
[z-engine] fold landed: 139865 events, mark 9185639 of C:\Users\…\eqlog_Primitive_freeport.txt, now live
[z-engine] views: combat.live 1 frames (1 reset / 0 diff), 2 rows, 0 ops, 397 B (widest 397 B); fold->frame mean 179 us max 179 us over 1
...
$ cd engine
$ { printf '%s\n' "$TOKEN"; sleep 20; } | cargo run -q -p zengine
...
const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'zengine-manual-'))
...
const engine = spawn(path.join(ROOT, 'engine/target/release/zengine.exe'), {
...
# staged C:\Users\…\Temp\zengine-manual-MWaZP7\eqlog_Primitive_freeport.txt (9185240 bytes)
...
<- {"epoch":2,"kind":"epoch","reason":"attach"}
<- {"id":3,"kind":"reply","ok":true,"result":{"accepted":true,"epoch":2}}
[z-engine] ingest: spell db built in 386 ms
<- {"epoch":2,"kind":"epoch","progress":{"events":15932,"pct":11.415401230670074},"reason":"progress"}
[z-engine] fold landed: 139860 events, mark 9185240 of C:\Users\…\eqlog_Primitive_freeport.txt, now live
...
# staged C:\t478\zengine-478-6ikaTi\eqlog_Primitive_freeport.txt (9185240 bytes)
EQC-ENGINE PORT=64299 PROTOCOL=1
-> {"op":"hello","token":"0f7d…7089","protocolVersion":1}
-> {"id":1,"op":"session.health","params":{}}
-> {"id":2,"op":"module.snapshot","params":{"module":"leveling"}}
-> {"id":3,"op":"session.attach","params":{"logPath":"C:\\t478\\zengine-478-6ikaTi\\eqlog_Primitive_freeport.txt"}}
<- {"engineVersion":"0.1.0","kind":"hello","ok":true,"protocolVersion":1}
...
<- {"epoch":2,"kind":"epoch","reason":"attach"}
<- {"id":3,"kind":"reply","ok":true,"result":{"accepted":true,"epoch":2}}
[z-engine] ingest: spell db ready in 403 ms
<- {"epoch":2,"kind":"epoch","progress":{"events":15932,"pct":11.415401230670074},"reason":"progress"}
...
[z-engine] fold landed: 139860 events, mark 9185240 of C:\t478\zengine-478-6ikaTi\eqlog_Primitive_freeport.txt, now live
<- {"epoch":2,"kind":"epoch","progress":{"events":139860,"pct":100.0},"reason":"progress"}
...
<- {"id":7,"kind":"reply","ok":true,"result":{"module":"character","seq":37,"state":{"character":{"logPath":"C:\\t478\\zengine-478-6ikaTi\\eqlog_Primitive_freeport.txt","name":"Primitive","server":"freeport"},"level":{"level":42,"source":"ding","ts":178 …
<- {"error":{"code":"notFound","message":"this engine folds no module named \"loot.ledger\""},"id":8,"kind":"error","ok":false}
<- {"id":9,"kind":"reply","ok":true,"result":{"epoch":2,"events":139860,"lastEventTs":1785795360000,"mark":{"log":"C:\\t478\\zengine-478-6ikaTi\\eqlog_Primitive_freeport.txt","offset":9185240},"status":"live","uptimeMs":1304}}
```
...
# staged C:\Users\…\Temp\zengine-487-2VZKHE\eqlog_Primitive_freeport.txt (9185424 bytes)
...
<- {"id":7,"kind":"reply","ok":true,"result":{"subscribed":true,"subscription":7}}
<- {"epoch":2,"id":7,"kind":"reset","rows":[],"total":0}
[z-engine] ingest: spell db ready in 392 ms
...
<- {"epoch":2,"kind":"epoch","progress":{"events":139863,"pct":100.0},"reason":"progress"}
[z-engine] fold landed: 139863 events, mark 9185424 of C:\Users\…\eqlog_Primitive_freeport.txt, now live
[z-engine] views: timers.rows 1 frames (1 reset / 0 diff), 1 rows, 0 ops, 440 B (widest 440 B); fold->frame mean 8.8 ms max 8.8 ms over 1
...
The integration suites spawn the built binary (`CARGO_BIN_EXE_zengine`) and drive the whole contract
