...
//! `CARGO_BIN_EXE_zengine` as a child process, hands it a token on stdin, parses the announce line
...
impl Engine {
    ...
    /// The engine process's id, for readings the protocol does not carry (the working set).
    #[must_use]
...
fn spawn_with(stderr: Stdio) -> (Child, ChildStdin) {
    let mut child = Command::new(env!("CARGO_BIN_EXE_zengine"))
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .stderr(stderr)
        .spawn()
