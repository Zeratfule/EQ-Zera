...
//!
//! The two bounds are also stated in `zengine/src/budgets.rs`, since a binary crate cannot be
...
/// The working-set ceiling at the landing, in bytes (Z Engine program step 4, 2026-09-07). Judged
/// here alone: the app reads the engine's working set natively and shows it beside the two panel
/// budgets, so the binary has no row for it. Set from the app-level measurements, not from this
/// corpus: the owner's 142 MB log lands at ~450 MB resident (release, Windows) and a bare engine
/// idles near 90 MB, so the 8 MB synthetic corpus cannot honestly reach 512 MB unless retained
/// state grows per event (an uncapped ring, a leak) - which is what this trips on, never allocator
/// noise or another machine's page accounting. Its first CI run prints the reading it judges.
const MAX_LIVE_WORKING_SET_BYTES: u64 = 512 << 20;
...
struct Measured {
    ...
    /// The engine's resident working set at the landing, read off the OS (the protocol does not
    /// carry it); `None` where the reading is unavailable.
...
[budget] {label}"
        ...
        match self.working_set_bytes {
            Some(bytes) => println!(
                "[budget]   memory    {:.1} MB resident at the landing (ceiling {} MB)",
                bytes as f64 / 1_048_576.0,
                MAX_LIVE_WORKING_SET_BYTES >> 20
            ),
            None => println!("[budget]   memory    no reading (the OS did not answer)"),
        }
    }
}

/// The resident working set of a process, in bytes, asked of the OS's own table: `tasklist` on
/// Windows (`Mem Usage`, in K), `ps` elsewhere (RSS, in KiB). A test-side reading only: the
/// engine spawns nothing for a number, and the app reads it natively.
fn working_set_of(pid: u32) -> Option<u64> {
    ...
    let field = if cfg!(windows) {
        // "zengine.exe","1234","Console","1","123,456 K"
        text.trim().rsplit(',').next()?.trim_matches('"').to_owned()
    } else {
        text.trim().to_owned()
    };
...
fn measure(log: &Path, patience: std::time::Duration) -> Measured {
    ...
    let working_set_bytes = engine.pid().and_then(working_set_of);
...
fn the_synthetic_fold_stays_inside_its_ci_budget() {
    ...
    if let Some(bytes) = m.working_set_bytes {
        assert!(
            bytes <= MAX_LIVE_WORKING_SET_BYTES,
            "WORKING SET BUDGET BREACHED: {:.1} MB resident at the landing, ceiling is {} MB. The \
             ceiling sits far above what this corpus can honestly reach, so a retained-state \
             defect (an uncapped ring, a leak per event) is what trips it - read it as such.",
            bytes as f64 / 1_048_576.0,
            MAX_LIVE_WORKING_SET_BYTES >> 20
        );
    }
...
///
/// `zengine/src/budgets.rs` states the same two numbers and cannot be imported here (binary crate),
