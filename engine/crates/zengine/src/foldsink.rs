...
impl FoldSink {
    ...
    pub fn new(inputs: &SinkInputs<'_>) -> Self {
        ...
        let mut fold = fold::Fold::new(registry_for(inputs, launch_ms), launch_ms)
            .with_combat_lane(combat_for(inputs), None);
...
impl EventSink for FoldSink {
    ...
    fn tick(&mut self, now_ms: i64) {
        ...
        if !self.live {
            if let Some(lane) = self.fold.lane.as_ref() {
                lane.set_live();
            }
        }
        self.live = true;
    ...
    fn define(&mut self, family: &str, payload: &serde_json::Value) -> bool {
        self.fold.define(family, payload)
    }
    ...
    fn restore(&mut self, ck: &fold::checkpoint::FoldCheckpoint) -> Result<(), String> {
        self.fold.restore(ck)
    }
    ...
    fn session_mark(&mut self, at: i64) -> bool {
        self.fold
            .lane
            .as_ref()
            .is_some_and(|lane| lane.session_mark(at))
    }
    ...
    fn combat_snapshot(&self, opts: &CombatOpts) -> Option<CombatSnapshot> {
        let lane = self.fold.lane.as_ref()?;
