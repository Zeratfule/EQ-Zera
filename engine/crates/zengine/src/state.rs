...
pub const RESIST_LEDGER: &str = "resist-ledger.json";

/// `<userData>/message-overlay.json`.
pub const MESSAGE_OVERLAY: &str = "message-overlay.json";
...
mod tests {
    ...
    fn scratch(name: &str) -> PathBuf {
        let dir = std::env::temp_dir().join(format!("zengine-state-{name}-{}", std::process::id()));
