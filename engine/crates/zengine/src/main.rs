//! `zengine` — the engine process: spawned, handed a secret, talked to, and killed. `session.attach`
...
fn main() -> ExitCode {
    ...
    // THE SPELL DB, PARSED WHILE THE APP IS STILL SHAKING HANDS (Z Engine, 2026-09-07, program step
    // 3). `spelldb::shared()` is one parse per process (~0.45 s here), and every attach waited for
    // it. Started now, on a named thread, it is usually done before the first attach arrives -
    // the attach's own call blocks on the same cell if it is not - and a warm start (~1.1 s) no
    // longer spends a third of itself on it.
    let _warming = std::thread::Builder::new()
        .name("zengine-spelldb".to_owned())
        .spawn(|| {
            let _db = eqlog::spelldb::shared();
        });
