...
struct Args {
    ...
    tz: Option<String>,
    /// The character, stated - for a LIVE log (`eqlog_<Name>_<server>.txt`), whose name carries
    /// no slice segment and so falls outside the fixture-shaped rule `eqlog::character_of` reads.
...
fn parse_args() -> Result<Args, String> {
    let (mut log, mut golden, mut tz, mut character) = (None, None, None, None);
    let (mut snapshots, mut stages) = (false, false);
    let mut it = std::env::args().skip(1);
    while let Some(a) = it.next() {
        match a.as_str() {
            "--golden" => golden = Some(it.next().ok_or("--golden needs a path")?),
            "--tz" => tz = Some(it.next().ok_or("--tz needs an IANA zone name")?),
            "--character" => character = Some(it.next().ok_or("--character needs a name")?),
    ...
    Ok(Args {
        log: log.ok_or(
            "usage: parity <logfile> [--golden <path>] [--snapshots] [--stages] [--tz <zone>] [--character <name>]",
        )?,
        golden,
        tz,
        character,
...
fn stages(
    ...
    let mut parsed = 0u64;
    {
        let (mut ev, mut seq) = (eqlog::event::Ev::new(), 0i64);
