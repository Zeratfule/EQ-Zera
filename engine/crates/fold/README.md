...
**THE REGISTRY IS PLUGGED IN** (JOS-478). `zengine` builds it on its ingest thread — one
`ClusterDeps` assembled from the parser's own catalog and clock, in `engine/crates/zengine/src/
