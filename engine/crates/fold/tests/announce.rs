...
//! event whose every field is known. `zengine`'s `tests/live_surfaces.rs` makes the same claim from
...
#[test]
fn hails_announces_on_the_greeting_and_on_nothing_else_you_could_say() {
    let mut p = Probe::new();
    let hailed = p.fold(r#"{"kind":"hail","seq":1,"ts":1000,"raw":"h","npc":"Beur Tenlah"}"#);
    assert!(hailed.contains(&"hails"));
    // A melee round is not a quest step.
    assert!(!p.fold(MELEE_HIT).contains(&"hails"));
}

#[test]
fn faction_announces_on_a_delta_and_on_a_rail_alike() {
    let mut p = Probe::new();
    let adjusted = p.fold(
        r#"{"kind":"factionHit","seq":1,"ts":1000,"raw":"f","faction":"Solusek Mining Co.","delta":5}"#,
    );
    assert!(adjusted.contains(&"faction"));
    // A saturation states no magnitude — but it IS a fact about that faction, so the panel hears it.
    let railed = p.fold(
        r#"{"kind":"factionHit","seq":2,"ts":1100,"raw":"f","faction":"Inhabitants of Hate","cap":"min"}"#,
    );
    assert!(railed.contains(&"faction"));
    assert!(!p.fold(MELEE_HIT).contains(&"faction"));
}

#[test]
fn coin_announces_on_income_and_stays_quiet_for_a_payout_that_is_not_one() {
    let mut p = Probe::new();
    ...
    let sold = p.fold(
        r#"{"kind":"loot","seq":3,"ts":3000,"raw":"l","item":"Sapphire Necklace +4","source":"Cleric of Innoruuk","disposition":"sold","coins":{"platinum":125}}"#,
    );
    assert!(sold.contains(&"coin"));
    // A destroy payout is a refund on something you already had, not income.
    let payout =
        p.fold(r#"{"kind":"coin","seq":4,"ts":4000,"raw":"c","source":"item","coins":{"gold":1}}"#);
    assert!(!payout.contains(&"coin"));
    // …and ordinary kept loot moves nothing here.
    let kept = p.fold(r#"{"kind":"loot","seq":5,"ts":5000,"raw":"l","item":"Bone Chips"}"#);
    assert!(!kept.contains(&"coin"));
}

#[test]
fn skills_announces_on_a_tick_with_a_value_and_on_one_without() {
    let mut p = Probe::new();
    let valued =
        p.fold(r#"{"kind":"skillUp","seq":1,"ts":1000,"raw":"s","skill":"Meditate","value":57}"#);
    assert!(valued.contains(&"skills"));
    // A value-less tick still says the skill rose, which is a change to `ups`.
    let bare = p.fold(r#"{"kind":"skillUp","seq":2,"ts":1100,"raw":"s","skill":"Meditate"}"#);
    assert!(bare.contains(&"skills"));
    assert!(!p.fold(MELEE_HIT).contains(&"skills"));
}

#[test]
fn deaths_announces_on_the_death_and_never_on_the_hits_it_is_ringing() {
    let mut p = Probe::new();
    // The ring is not published state, so absorbing a hit says nothing.
    let hit = p.fold(
        r#"{"kind":"damage","seq":1,"ts":1000,"raw":"d","attacker":"a fire giant","target":"You","amount":106,"skill":"Melee","dtype":"melee","crit":false}"#,
    );
    assert!(!hit.contains(&"deaths"));
    let resisted = p.fold(
        r#"{"kind":"resist","seq":2,"ts":1500,"raw":"r","caster":"a fire giant","target":"You","spell":"Fear","incoming":true}"#,
    );
    assert!(!resisted.contains(&"deaths"));
    // The death is the row.
    let died =
