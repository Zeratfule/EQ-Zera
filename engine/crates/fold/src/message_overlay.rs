...
pub struct MessageOverlayMiner {
    ...
    /// An input, not state: skipped by a checkpoint and carried over on restore.
    #[serde(skip)]
    facts: SpellFacts,
}

impl MessageOverlayMiner {
    /// Take the facts from the miner a checkpoint replaced - the one input a restored miner lacks.
    pub fn carry_facts(&mut self, from: &Self) {
        self.facts = from.facts.clone();
    }
