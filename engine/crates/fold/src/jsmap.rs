...
impl<V> JsMap<V> {
    ...
    /// The row for `key`, made by `make` and appended when absent - ONE hash, where the
    /// `contains_key` / `insert` / `get_mut` spelling the fold used on every hit paid three, and an
    /// owned key only on the miss (Z Engine, 2026-09-06).
    pub fn get_or_insert_with(&mut self, key: &str, make: impl FnOnce() -> V) -> &mut V {
        let i = match self.at.get(key) {
            Some(&i) => i,
            None => {
                let i = self.entries.len();
                self.at.insert(key.to_string(), i);
...
impl<'de, V: serde::Deserialize<'de>> serde::Deserialize<'de> for JsMap<V> {
    /// The mirror of `Serialize`: a map, in order, rebuilt through `insert` so the index and
    /// the entries agree - the fold checkpoint's road (checkpoint.rs).
