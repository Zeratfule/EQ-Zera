...
/// `zengine::spells`, which owns the install directory both sit in.
pub mod dbstr;
pub mod epoch;
pub mod event;
pub mod jsfn;
...
/// directory belong to `zengine::spells`. Nothing in the fold reads it — a fold that never needs
/// the client table can be replayed, shipped and re-estimated without one.
pub mod spells_us;
mod wiring;
pub use wiring::{registered, ClusterDeps, PersistedState};
...
pub trait EqModule {
    ...
    /// The module's whole state, serialized, for a fold checkpoint (`checkpoint.rs`). `None` =
    /// this module does not checkpoint, which makes the whole checkpoint unavailable.
    fn checkpoint(&self) -> Option<Vec<u8>> {
        None
    }

    /// The inverse: replace this module's state with what `checkpoint` produced. `false` refuses
    /// bytes this build does not read, and the caller discards the registry.
...
pub const WIRING_ORDER: &[&str] = &[
    ...
    "consider",
    "resist",
    // The five surfaces the log could always have answered and nothing published (Z Engine,
    // 2026-09-08). Appended rather than slotted beside their kin: registration order is bus
    // delivery order, and none of these is read by any module that folds after it, so the end of
    // the list is where a new module belongs until something needs it earlier.
    "hails",
    "faction",
    "coin",
    "skills",
...
impl Registry {
    ...
    ///
    /// The one caller is `zengine::foldsink`, and only when the attach carried a `stateDir`.
    ...
    ///
    /// The one caller is the production construction, `zengine::foldsink::registry_for`, right
...
pub struct Fold {
    ...
    /// The engine on its own thread instead (Z Engine, 2026-09-07) - see `lane.rs`. The app uses
    /// this; the parity tool and the tests keep `combat` inline, and `tests/lane_parity.rs` holds
    /// the two to the same answer.
    pub lane: Option<lane::CombatLane>,
...
impl Fold {
    ...
    /// A define, to the registry - and, for the roster family, to the lane's own roster, in order
    /// with the events around it.
    pub fn define(&mut self, family: &str, payload: &Value) -> bool {
        let took = self.registry.define(family, payload);
        if family == "roster" {
            if let Some(l) = &self.lane {
                l.define_roster(payload);
            }
        }
        took
    }
...
mod tests {
    ...
    // `zengine` proves the push travels.
