...
pub struct TimelineRaw {
    ...
    pub kind: crate::checkpoint::Static,
    /// `miss` / `resist` for avoided and resisted instants; `None` = a landed hit.
    #[serde(default, deserialize_with = "crate::checkpoint::interned_opt")]
    pub outcome: crate::checkpoint::StaticOpt,
...
pub struct StanceRaw {
    #[serde(deserialize_with = "crate::checkpoint::interned")]
    pub group: crate::checkpoint::Static,
    pub name: String,
    pub start: i64,
    pub end: Option<i64>,
}
...
pub struct Encounter {
    ...
    /// A DEQUE, not a Vec (Z Engine, 2026-09-06): the cap drops the oldest, and a Vec's `remove(0)`
    /// moved the other 7,999 records (1.4 MB) on EVERY hit of a fight past the cap - measured as the
    /// largest single cost in the whole fold. Order, cap and contents are unchanged.
    pub events: VecDeque<TimelineRaw>,
    ...
    pub markers: VecDeque<MarkerRaw>,
