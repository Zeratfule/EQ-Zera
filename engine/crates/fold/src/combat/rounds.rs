...
pub struct RoundAccum {
    ...
    pub excluded: [i64; 4],
    /// Scratch for the `verb|target` key, so a swing whose lane already exists allocates nothing
    /// (Z Engine, 2026-09-06: `add` ran on every one of a million swings, twice, and built four
    /// strings each time). Never read outside `add`.
    key: String,
}
...
impl RoundAccum {
    ...
    pub fn add<S: AsRef<str>>(&mut self, rec: &SwingRecord<'_, S>) {
        ...
        self.key.clear();
        self.key.push_str(&verb);
        self.key.push('|');
        self.key.push_str(&folded(rec.target.trim()));
        let token = signature_token(rec.amount, rec.avoided);
        match self.pending.get_mut(&self.key) {
            Some(lane) => {
                // The last swing's label wins, as before; the write is skipped when it already has.
                if lane.skill != rec.skill {
