...
#[derive(Debug, Clone)]
enum Body<'a> {
    /// Borrowed from the parser on the production path; owned when the event was copied to cross
    /// to the combat lane (`Event::to_static`). Every reader derefs the same way.
    Typed(std::borrow::Cow<'a, Payload>),
    Json(Value),
}
...
impl<'a> Event<'a> {
    ...
    pub fn typed(p: &'a Payload) -> Event<'a> {
        Event {
            kind: p.kind(),
            body: Body::Typed(std::borrow::Cow::Borrowed(p)),
        }
    }
}
...
impl Event<'_> {
    ...
    /// A coin object — `coin.coins`, `purchase.price`, and the price a sold loot line states.
    ///
    /// The pairs come back in the order the CLAUSE named its denominations, which is what the
    /// writer preserved and what a display that echoes the sentence needs. `None` for an absent
    /// field; `Some(&[])` for the present-and-empty object a free price writes, which is a
    /// different claim (`purchase` has said so since JOS-144).
    ///
    /// The pairs are collected rather than borrowed, because the two bodies cannot hand back the
    /// same slice: the typed half already holds `(&'static str, i64)` pairs, while a JSON body's
    /// numbers have to be read out of `Value`s. A coin sentence names at most four denominations,
    /// so the `Vec` is the cheapest thing that lets both answer.
    #[must_use]
    pub fn coins(&self, key: impl EvKey) -> Option<Vec<(&str, i64)>> {
        let k = key.key()?;
        match &self.body {
            Body::Typed(p) => p
                .coins(k)
                .map(|pairs| pairs.iter().map(|(d, v)| (*d, *v)).collect()),
            Body::Json(v) => {
                let obj = v.get(k.as_str())?.as_object()?;
                Some(
                    obj.iter()
                        .filter_map(|(d, n)| Some((d.as_str(), n.as_i64()?)))
                        .collect(),
                )
            }
        }
