...
mod core_cell;
pub use core_cell::{empty_core, shared_core};
...
/// The shared core on a checkpoint: serialized through the cell, read back into a fresh cell
/// (`BuffsModule::restore` then moves the contents into the cell `buffTimers` still shares).
mod core_serde {
    use super::{BuffsCore, SharedCore};
    pub fn serialize<S: serde::Serializer>(core: &SharedCore, s: S) -> Result<S::Ok, S::Error> {
        serde::Serialize::serialize(&*core.borrow(), s)
    }
    pub fn deserialize<'de, D: serde::Deserializer<'de>>(d: D) -> Result<SharedCore, D::Error> {
        let core: BuffsCore = serde::Deserialize::deserialize(d)?;
        Ok(std::rc::Rc::new(std::cell::RefCell::new(core)))
    }
}
...
impl EqModule for BuffsModule {
    ...
    fn restore(&mut self, bytes: &[u8]) -> bool {
        let Some(mut fresh) = crate::checkpoint::parse::<Self>(bytes) else {
            return false;
        };
        // The core is SHARED with buffTimers through the Rc: the restored contents move into the
        // cell both modules hold, and the inputs (the spell facts, here and inside the core and the
        // miner) are carried over from the module being replaced.
        let db = std::mem::take(&mut self.core.borrow_mut().stats.db);
        std::mem::swap(&mut *self.core.borrow_mut(), &mut *fresh.core.borrow_mut());
        self.core.borrow_mut().stats.db = db;
        fresh.core = Rc::clone(&self.core);
        fresh.facts = std::mem::take(&mut self.facts);
        fresh.mining.carry_facts(&self.mining);
        *self = fresh;
        true
    }
