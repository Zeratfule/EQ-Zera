...
impl SessionDetector {
    ...
    /// `(evidence_ts, camp_ts)` - the whole of this detector's state, for a checkpoint.
    pub fn state(&self) -> (i64, i64) {
        (self.evidence_ts, self.camp_ts)
    }

    pub fn set_state(&mut self, (evidence_ts, camp_ts): (i64, i64)) {
        self.evidence_ts = evidence_ts;
        self.camp_ts = camp_ts;
    }
