...
- **An untagged union with `deny_unknown_fields` cannot answer `unknownOp`.** A request naming an
  ...
  (zengine's `ops::classify`). Known-op lists come from the generated tag enums, never literals.
...
- **`zengine` IS A BINARY CRATE, so an integration test cannot import a constant from it.** There is
...
- **Phase 4 — post-cutover**: **BEGUN.**
  - ~~budgets in CI against a pinned fixture~~ **DONE** (JOS-501): `engine/crates/zengine/tests/
...
- ~~**RULING 19'S LAST TWO OPS**~~ — **DONE (JOS-502), and surface 8 is complete.** `perf.budgets`
  serves the definitions AND the live verdict off `zengine/src/budgets.rs` (the same two ceilings
