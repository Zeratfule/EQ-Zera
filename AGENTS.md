...
- **FLAKES ARE TRACKED AND FIXED, regardless of who wrote the test** (owner
  ...
  - `zengine tests/perf_snapshot.rs` · `perf.snapshot was refused: Unavailable
  ...
  - `zengine tests/combat.rs` live-meter tests · the fight closes before the
  ...
  - `zengine/tests/combat.rs` live-meter current-kind · expected `current`,
...
`src/main/dataServer/README.md`, and the engine's own side in `engine/crates/zengine/README.md`.

```
EverQuest client ──appends──► log file ──► ENGINE (engine/crates/zengine, below-normal priority)
...
- **the engine's own budgets** — `engine/crates/zengine/tests/budget.rs`, `npm run budget:ci` in CI
...
- **THE ENGINE SHIPS SIGNED, AND THE MATCHER SHAPE IS WHAT SIGNS IT (JOS-473).**
  ...
  resolution) builds `zengine.exe` RELEASE; `extraResources` copies it to
  `resources/engine/zengine.exe` — outside the asar, where a native exe can
