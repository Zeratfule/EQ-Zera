...
// bitmap (dark background + an accent-coloured "EQ" glyph) encoded to PNG by hand (zlib is in
...
// game art): a dark rounded panel with a violet serif-ish "EQ".
...
/** A palette token as the opaque RGBA tuple the rasteriser fills with. */
function rgba(hex: string): RGBA {
  const n = parseInt(hex.slice(1, 7), 16)
  return [n >> 16, (n >> 8) & 255, n & 255, 255]
}

const BG = rgba(PALETTE.bg) // matches the app window background
const PANEL = rgba(PALETTE.paper) // the inner panel — the app's card surface
const GLYPH = rgba(PALETTE.accent) // the "EQ" glyph, in the app accent
const GLYPH_HI = rgba(PALETTE.accentHi)
...
function drawMaster(): Canvas {
  ...
  const eX = 28
  const eW = 44
  cv.rect({ x0: eX, y0: top, x1: eX + strokeW, y1: bot }, GLYPH) // vertical spine
  cv.rect({ x0: eX, y0: top, x1: eX + eW, y1: top + strokeW }, GLYPH) // top bar
  cv.rect({ x0: eX, y0: midY - strokeW / 2, x1: eX + eW - 7, y1: midY + strokeW / 2 }, GLYPH) // mid bar
  cv.rect({ x0: eX, y0: bot - strokeW, x1: eX + eW, y1: bot }, GLYPH) // bottom bar
  cv.rect({ x0: eX, y0: top, x1: eX + 4, y1: bot }, GLYPH_HI) // highlight along the spine

  // --- Q ---
  const qcx = 120
  const qR = 40
  cv.disc(qcx, midY, { outer: qR, inner: qR - strokeW }, GLYPH) // ring
  cv.disc(qcx, midY, { outer: qR, inner: qR - 4 }, GLYPH_HI) // thin bright outer edge
  ...
  for (let t = 0; t < 14; t++) {
    cv.rect({ x0: qcx + 14 + t, y0: midY + 14 + t, x1: qcx + 14 + t + strokeW, y1: midY + 14 + t + strokeW }, GLYPH)
  }

  // --- C --- (the Q's ring with its right side opened — erased back to panel color)
  const ccx = 200
  const cR = 40
  cv.disc(ccx, midY, { outer: cR, inner: cR - strokeW }, GLYPH)
  cv.disc(ccx, midY, { outer: cR, inner: cR - 4 }, GLYPH_HI)
