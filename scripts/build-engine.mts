/**
 * build-engine.mts — the RELEASE `zengine.exe` the installer ships (JOS-473, phase 3), and the
 ...
 *  the file that becomes `resources/engine/zengine.exe` in a packaged app. The NAME comes from the
 ...
 */
/**
 * Below this much free memory the release compile of the fold crate (three hundred serde derives,
 * sixteen codegen units by default) has been seen killed on the owner's machine; one codegen unit
 * and one job finish where the default did not, at roughly twice the wall time.
 */
const LOW_MEMORY_BYTES = 8 * 1024 * 1024 * 1024

/** The cargo environment for this machine's headroom: the low-memory shape when it is tight. */
export function cargoEnv(): NodeJS.ProcessEnv {
  if (freemem() >= LOW_MEMORY_BYTES) return process.env
  console.log(
    `build:engine: ${String(Math.round(freemem() / 1048576))} MB free; building with one job and one codegen unit`
  )
  return { ...process.env, CARGO_BUILD_JOBS: '1', CARGO_PROFILE_RELEASE_CODEGEN_UNITS: '1' }
}

export function buildEngineRelease(): string {
  const res = spawnSync(cargoBinary(), ['build', '--release', '-p', 'zengine'], {
    cwd: ENGINE_DIR,
    stdio: 'inherit',
    env: cargoEnv()
  })
  ...
  if (res.status !== 0) {
    throw new Error(`build:engine: cargo build --release -p zengine failed (exit ${String(res.status)})`)
  }
