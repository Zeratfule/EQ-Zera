...
- v1.20.1 (2026-09-09): the first SIGNED release. Azure Trusted Signing (Microsoft now calls it Artifact Signing), account `eqzera` in resource group `eq-zera-signing`, certificate profile `eqzera-public`, type PublicTrust, built on an Individual public identity validation; the certificate subject CN is `Jack Thomas`, which is now the active `win.signtoolOptions.publisherName` in `electron-builder.yml`, so electron-updater refuses any update not signed by that identity (`ERR_UPDATER_INVALID_SIGNATURE`) and the GitHub account is no longer the sole trust root. CI signs whenever the repository variable `SIGNING_ENABLED` is `true`, set that day; the variable exists because v1.19.4 had failed to build against a certificate profile that did not exist yet. The Organization-type validation was the wrong path - Microsoft came back asking for business registration documents that a personal fork does not have - and the Individual path, which lives behind the Organizations/Individuals dropdown on the portal's Identity validations blade, completed in minutes once it was found. SmartScreen's "Windows protected your PC" is expected to stop, and the installer's UAC/SmartScreen dialogs now name Jack Thomas as publisher. Certificates rotate every few days on their own; the only manual step left is renewing the identity validation in about a year. SECURITY.md, SETUP.md, `release.yml`'s header and `src/main/updater.ts`'s fork-switch comment now describe the signed pipeline.
