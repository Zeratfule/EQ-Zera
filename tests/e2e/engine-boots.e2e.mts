 ...
 * between two: Electron main spawns `zengine.exe` (real Rust, built from this worktree), hands it a
 ...
 * drives every failure path with no app and no Rust, and `engine/crates/zengine/tests/
 ...
 *   * the engine EXISTS — `wmic` names the `zengine.exe` whose PARENT is this launch's Electron.
 ...
 *     the app, and then no `zengine.exe` of ours left in the process table.
...
async function stepSpawned(launch: FixtureLaunch, before: readonly number[]): Promise<number | null> {
  ...
  check(
    'launching with EQC_ENGINE=1 spawns exactly one zengine.exe under this launch',
    found.pids.length === 1,
...
async function stepQuit(launch: FixtureLaunch, out: AppOutput, pids: readonly number[]): Promise<void> {
  ...
  check(
    'no zengine.exe outlives the app that spawned it',
    orphans.length === 0,
...
async function main(): Promise<void> {
  ...
  const before = engineTable().pids
  if (before.length > 0) {
    note(`${String(before.length)} zengine.exe already running before this spec launched anything — they are excluded, not killed`)
  }
