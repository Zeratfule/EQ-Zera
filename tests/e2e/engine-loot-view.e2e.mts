...
/**
 * Is every row of `before` still there, in the same order, in `after`?
 *
 * A SUBSEQUENCE, NOT AN EQUALITY, and the difference is that this spec runs against a LIVE app.
 * The two readings are taken minutes apart with the startup replay still delivering loot, so the
 * ledger legitimately gains rows between them (measured: 29 rows before the flip, 30 after) and a
 * deep-equal was asserting that time had not passed. What the toggle actually promises is that
 * flipping back RESTORES the app-fed ledger: nothing the app drew went missing, and nothing got
 * reordered. New rows arriving at the top of a newest-first table break neither.
 *
 * Returns the index of the first row of `before` that could not be matched, or -1 when they all
 * were, so the failure line can name the row rather than the count.
 */
function missingRow(before: Ledger, after: Ledger): number {
  let at = 0
  for (let i = 0; i < before.length; i += 1) {
    const want = JSON.stringify(before[i])
    while (at < after.length && JSON.stringify(after[at]) !== want) at += 1
    if (at >= after.length) return i
    at += 1
  }
  return -1
}
  ...
  try {
    ...
    if (await stepFlatLedger(page)) {
      ...
      if (await stepBrokered(page)) {
        ...
        const lost = missingRow(app, back)
        check(
          'flipping back restores the app-fed ledger: every row it drew is still there, in order',
          lost < 0,
          lost < 0
            ? `all ${String(app.length)} rows back, in order (app ${String(app.length)} · back ${String(back.length)})`
            : `row ${String(lost)} never came back: ${JSON.stringify(app[lost])}`
        )
        // …and the ledger did not SHRINK. The replay may still be delivering loot, so it is free to
        // have grown; losing rows to a component that was unmounted is the defect.
        check(
          'the ledger that came back is no shorter than the one that went away',
