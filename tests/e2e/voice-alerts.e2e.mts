...
/**
 * Answer the analytics first-run notice, the alert-dialog-focus.e2e.mts idiom — and for the SAME
 * reason that spec spells out, which has now bitten twice.
 *
 * A fresh `userData` always shows it, and it is a MUI `Snackbar` pinned to the app's bottom edge:
 * a Snackbar outranks a modal menu in MUI's z-index scale, so any menu item that opens down there
 * is unclickable and the failure reads as "the say picker did nothing" rather than as "a first-run
 * strip was in front of it". Whether a given menu lands on the strip is a function of how far down
 * the page its anchor sits, which is exactly the kind of thing an unrelated change moves: the
 * alert list gained the two seeded wish-list defs, §7's row moved two rows down the list, and its
 * say-picker menu started opening over the notice. Answering the notice up front takes the whole
 * class of failure off the table rather than nudging one menu back above it.
 */
