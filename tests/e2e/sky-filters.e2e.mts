...
async function stepShowAllSurvivesInteraction(page: Page): Promise<void> {
  ...
  await mount.run('the show-all interaction step', async () => {
    ...
    // AND THE ROW GOES BACK CLOSED, for exactly the reason the star goes back two lines up. This
    // step hands the list on to `stepCollapsedRowsDrawNoPanel`, whose whole subject is a list with
    // NO panel mounted in it, so an accordion left open here is not this step's claim — it is a
    // booby trap for that one. It was a real red run on 2026-09-08 (three failures, all of them
    // "one more panel than expected"): the row opened above is #95 of 95 while "show all" is on,
    // and whether the page cap two steps later still holds that quest depends on where the LIVE
    // replay has sorted it by then, which is nobody's promise. No settle is needed after the
    // click — the next step opens with `settleStable` on exactly this count, which is what waits
    // the Collapse's exit transition out.
    await page.locator(`${ROW}.Mui-expanded .MuiAccordionSummary-expandIconWrapper`).first().click({ timeout: 15_000 })
    return true
  })
}
