...
async function stepCanvas(page: Page): Promise<void> {
  ...
  // A PIXEL OF SLACK, AND NO MORE (2026-09-08). MapCanvas multiplies its FRACTIONAL measured box
  // by the dpr (`prepare()`: `Math.round(size.w * dpr)`), while `cssW`/`cssH` above are that box
  // already rounded to whole pixels — so on a display whose dpr is not a whole number the two
  // roundings can disagree by one: measured here at dpr 2.25, a 576.44px pane gives a 1297px
  // buffer where `round(576 * 2.25)` says 1296. One pixel is the whole of that disagreement; the
  // bug this check exists for is a buffer at CSS size, which at this dpr is 1296 pixels out.
  check(
    'the canvas backing store is scaled by devicePixelRatio (a CSS-sized buffer is the blurry-map bug)',
    Math.abs(m.bufW - m.cssW * m.dpr) <= 1 && Math.abs(m.bufH - m.cssH * m.dpr) <= 1,
