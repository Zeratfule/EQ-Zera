...
async function stepUpdateChipClearsPreferences(page: Page): Promise<void> {
  // THE RAIL SCROLLS (NavDrawer: its cells outrun a short window and it hides its own bar), so
  // once the Zone Loot row joined (2026-09-07) the Preferences row sits below the fold at this
  // harness's window height. A hit test on a row that is off screen answers null at every point,
  // which says nothing about the tooltip. Bring it into view first, as a user scrolling would.
  await page.evaluate(() => document.querySelector('[data-testid="nav-preferences"]')?.scrollIntoView({ block: 'nearest' }))
