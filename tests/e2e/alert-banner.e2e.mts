...
/**
 * PUT THE WINDOW THROUGH MAIN'S OWN PLACEMENT BEFORE ANYTHING MEASURES IT (2026-09-08).
 *
 * `stepStripScalesWithText` reads this window, doubles the reading, and asks whether main answers
 * with twice the window. Main answers through `scaledStripBounds` -> `setBounds`, which multiplies
 * the stored LAYOUT BOX — and a window that has only ever been CONSTRUCTED has not been through
 * that door. On a display whose scale factor is not a whole number that difference is visible:
 * measured here with a bare Electron window and no app code in the way, at `scaleFactor: 2.25`,
 * `new BrowserWindow({width: 720, height: 260})` reports 724x263 while `setBounds(720x260)` on the
 * same window reports 720x260 exactly. Doubling 724 asked for a rectangle nothing had ever been
 * told to make, and main's honest 1441 read as a failure.
 *
 * Re-asserting the text scale it is ALREADY at re-places it: the shared route calls
 * `refitStripsForTextScale()` on every write rather than only on a change
 * (main/ipc/windowControls.ts), which is exactly the door the 200 % reading will come back through.
 */
async function stepPlacedByMain(app: ElectronApplication, banner: Page): Promise<void> {
  const before = await overlayBounds(app, 'alertBanner')
  await setOverlayTextScale(banner, 1)
  const placed = await settleStable(() => overlayBounds(app, 'alertBanner'), { timeoutMs: 15_000 })
  check(
    'the banner window is where main placed it, not where the constructor rounded it',
    placed !== null,
    `${String(before?.width)} -> ${String(placed?.width)}`
  )
}
...
async function main(): Promise<void> {
  ...
  try {
    ...
    if (banner) {
      ...
      await stepPlacedByMain(app, banner)
      ...
      // was a 0.72 nobody could reach. IT NEEDS A LINE ON SCREEN: the card whose alpha it measures
      // IS the banner line, and the one fired above has expired by now (`stepPerAlertSwitchHides`
      // below reads a banner with nothing on it, which is how long these steps take). Without this
      // the only thing painted in the window is the drag frame's fixed 0.65 and the step reads NaN.
      await preview(page, banner, 1)
