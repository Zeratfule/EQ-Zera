...
const SLACK = 2
/**
 * …AND A WINDOW RECTANGLE COSTS MORE THAN A PIXEL ON A FRACTIONALLY-SCALED DISPLAY (2026-09-08).
 *
 * `SLACK` above is about a layout box rounding to whole pixels. This one is about Windows: at a
 * 225 % display scale (`scaleFactor: 2.25`) the DIP -> device -> DIP round trip a BrowserWindow
 * rectangle makes is not the identity, and the error is not a constant. MEASURED on this machine
 * with a bare Electron window and no app code in the way:
 *
 *   setBounds 720x260  -> getBounds 720x260     (0)
 *   setBounds 1040x720 -> getBounds 1040x720    (0)
 *   setBounds 1440x520 -> getBounds 1441x520    (+1)
 *   setBounds 1280x860 -> getBounds 1282x860    (+2)
 *   setBounds  798x300 -> getBounds  799x300    (+1)
 *   new BrowserWindow(720x260) -> getBounds 724x263   (+4 / +3 — the CONSTRUCTOR rounds outward)
 *
 * So a WINDOW compared against a number this file computed is honest to about three pixels, and
 * four is the smallest margin that covers every reading above without swallowing a real one: the
 * defect these steps exist for is a card that kept its 100 % width at 200 %, which is HALF the
 * window, not four pixels of it. Only window-versus-computed comparisons use it; the stored layout
 * box, which is the app's own arithmetic and has no display in it, keeps `SLACK`.
 */
...
export async function stepWindowScalesWithText(
  ...
  // MEASURE FROM A WINDOW MAIN HAS PLACED, NOT FROM THE ONE THE CONSTRUCTOR MADE (2026-09-08).
  // Everything below multiplies the base and compares the product against what main answers, and
  // main answers through `scaledStripBounds` -> `setBounds`. A window that has only ever been
  // CONSTRUCTED has not been through that door: on this machine's 225 %-scaled display the
  // constructor rounds a 532px card out to a 538px window (see WINDOW_SLACK), so doubling the
  // reading asked for a rectangle nothing had ever been told to make, and main's honest answer
  // read as a failure. Re-asserting the scale it is already at re-places it
  // (`refitStripsForTextScale` runs on every write, not only on a change), which is exactly the
  // state the 150 % and 200 % readings are taken in.
  await setOverlayTextScale(card, 1)
  const start = await settleStable(() => overlayBounds(app, KIND), { timeoutMs: 15_000 })
  ...
  for (const scale of [1, 1.5, 2]) {
    ...
    const b = await settle(
      ...
      (r) => r !== null && Math.abs(r.width - want) <= WINDOW_SLACK,
      { timeoutMs: 15_000 }
    )
    ...
    if (
      !check(
        ...
        b !== null && Math.abs((b as Bounds).width - want) <= WINDOW_SLACK,
    ...
    check(
      ...
      Math.abs(big.x + big.width / 2 - (base.x + base.width / 2)) <= WINDOW_SLACK && big.y === base.y,
  ...
  await setOverlayTextScale(card, 1)
  await settle(() => overlayBounds(app, KIND), (r) => r !== null && Math.abs(r.width - base.width) <= WINDOW_SLACK, {
    timeoutMs: 15_000
  })
}
...
export async function stepResizeRecordsLayoutBox(app: ElectronApplication, card: Page): Promise<void> {
  ...
  // …and the other direction: back at 100%, the window IS that remembered box. WINDOW_SLACK
  // rather than SLACK because this comparison ends at a WINDOW: the box above is the app's own
  // arithmetic, but the rectangle it is put back into has been through Windows' DIP rounding twice
  // over (once on the drag's setBounds, once on the re-place) - see WINDOW_SLACK for the readings.
  await setOverlayTextScale(card, 1)
  const back = await settle(
    () => overlayBounds(app, KIND),
    (b) => b !== null && Math.abs(b.width - want) <= WINDOW_SLACK,
    { timeoutMs: 15_000 }
  )
  check(
    '…and turning the text back to 100% gives exactly that box as the window',
    back !== null && Math.abs((back as Bounds).width - want) <= WINDOW_SLACK,
