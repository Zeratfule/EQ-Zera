 ...
 * (overlay/overlayScale.tsx) and the transparency is the body's own `rgba(11,15,26,a)`; both are
...
function bodyAlpha(page: Page): Promise<number> {
  return page.evaluate(() => {
    ...
    for (const el of Array.from(document.querySelectorAll('div'))) {
      // PALETTE.bg is #0b0f1a (the 2026-09-06 "Midnight & arcane" theme) = rgb(11, 15, 26); the
      // overlays painted rgb(14, 17, 21) before it. Same rewrite as stripScaleSteps.mts.
      const m = /^rgba\(11, ?15, ?26, ?([\d.]+)\)$/.exec(getComputedStyle(el).backgroundColor)
