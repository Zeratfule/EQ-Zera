...
// `'zengine.exe'` here was a latent divergence from `engineProtocol.ts`'s platform-aware constant:
 ...
 * which is whether `zengine.exe` is stale.
 */
...
//
// TWO OUTPUTS, TWO GATES. `out-e2e/` is electron-vite's; `engine/target/release/zengine.exe` is
...
const ENGINE_DEP_INFO = join(ROOT, 'engine', 'target', 'release', 'zengine.d')
...
/**
 * Build `zengine.exe` if the inputs cargo would rebuild from have changed since it was written.
 *
...
export function buildEngineIfStale(): string {
  ...
  console.log('build: cargo build --release -p zengine (the engine binary is stale)…')
  ...
  if (res.status !== 0) {
    throw new Error(`cargo build --release -p zengine failed (exit ${String(res.status)})`)
  }
 ...
 * return, which is the whole trick: a checkout whose `out-e2e/` is fresh and whose `zengine.exe` is
