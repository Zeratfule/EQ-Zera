...
async function openSky(page: Page): Promise<boolean> {
  ...
  // SETTLED, NOT READ ONCE (2026-09-08). The filter bar and the counts line are two paints:
  // waitForSelector above returns on the FIRST, and reading the second in the same tick has
  // been losing that race - a bare null here aborts the whole spec as 'never reached the Sky
  // tab' when the tab is in fact up and half a frame from finishing.
  const counts = await settle(() => filteredCount(page), (n) => n !== null, { timeoutMs: 30_000 })
  return check('…with the counts line under it', counts !== null, String(counts))
}
