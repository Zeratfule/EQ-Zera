 ...
 *
 *   1. THE PROCESS TABLE — is there an `zengine.exe`, and whose child is it?
 ...
 *
 * POSITIVE IDENTIFICATION, THE `mainWindow()` RULE, APPLIED TO A PROCESS. "The zengine.exe that
 ...
 * `app.process().pid = 7308`, main = 52676 with parent 7308, `zengine.exe` = 45692 with parent
...
/** `tasklist` rows: `"zengine.exe","33324","Console","1","5,564 K"`. The fallback path — a pid list
...
export function engineTable(): EngineTable {
  ...
  // `zengine.exe` cannot answer the only question worth asking.
 ...
 *
 * The engine's answer is fixed by contract (zengine's README, clause 4): one
