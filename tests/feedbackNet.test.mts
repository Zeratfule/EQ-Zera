...
// EQ ZERA FORK: DARK BUILD. There is no compiled endpoint and no bucket; the two tests that
// pinned the upstream deployment are replaced by their negation.
test('the fork compiles NO feedback endpoint and NO bucket', () => {
  assert.equal(feedbackEndpointConfigured(), false)
  assert.equal(net.FEEDBACK_API_URL, '')
  assert.equal(net.FEEDBACK_S3_BUCKET, '')
})

test('the BOUND allowedUploadUrl refuses every host when no bucket is compiled in', () => {
  assert.equal(allowedUploadUrl('https://eqcompanion-logs-9f3a2c17.s3.us-east-1.amazonaws.com/'), null)
  assert.equal(allowedUploadUrl('https://s3.us-east-1.amazonaws.com/'), null)
...
test('THE PACKAGED PROOF: the gate is shut, and the env var does nothing at all', () => {
  ...
  const packaged = facts({ execPath: 'C:\\Users\\me\\AppData\\Local\\Programs\\eq-zera\\EQ Zera.exe' })
