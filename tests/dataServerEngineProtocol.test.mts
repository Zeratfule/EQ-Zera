...
// integrator ran `cargo test` in the owner's checkout — which writes `target/debug/zengine.exe` as
...
test('the binary is PROBED, and the dev tree contributes RELEASE unless somebody asks otherwise', () => {
  ...
  assert.deepEqual(found, [
    ...
    'C:/repo/engine/target/release/zengine.exe',
    ...
    'C:/app/resources/engine/zengine.exe',
    'C:/app/resources/zengine.exe'
  ])
})
...
test('a debug engine ANNOUNCES ITSELF, names the opt-in, and says how to undo it', () => {
  const notice = engineProfileNotice('C:/r/engine/target/debug/zengine.exe', {
    appPath: 'C:/r',
    resourcesPath: '',
    profile: 'debug'
  })
  ...
  assert.match(notice, /DEBUG engine at C:\/r\/engine\/target\/debug\/zengine\.exe/)
...
test('SILENCE IS FOR THE ORDINARY LAUNCH, and for the packaged one especially', () => {
  ...
  assert.equal(engineProfileNotice('C:/r/engine/target/release/zengine.exe', env), null)
  ...
  assert.equal(engineProfileNotice('RES/engine/zengine.exe', env), null)
  ...
  assert.equal(engineProfileNotice('C:/Users/x/AppData/Roaming/eqc/engine-run/zengine.exe', env), null)
})
...
test('the profile is read off the path cargo writes to, both spellings', () => {
  assert.equal(engineBinaryProfile('C:/r/engine/target/debug/zengine.exe'), 'debug')
  assert.equal(engineBinaryProfile('C:\\r\\engine\\target\\release\\zengine.exe'), 'release')
  // Not cargo's output at all: the packaged path, the staging directory, some other `target`.
  assert.equal(engineBinaryProfile('RES/engine/zengine.exe'), null)
  assert.equal(engineBinaryProfile('C:/app/target/debug/zengine.exe'), null)
  // …and `isCargoTargetBinary` is exactly this question asked as a yes/no, so the two cannot drift.
  for (const path of [
    'C:/r/engine/target/debug/zengine.exe',
    'C:/r/engine/target/release/zengine.exe',
    'RES/engine/zengine.exe',
    'C:/app/target/debug/zengine.exe'
  ]) {
...
// an app that spawns `engine/target/debug/zengine.exe` directly makes the next `cargo build -p
// zengine` fail at the LINK step — "Access is denied", not a compile error, which is the confusing
...
test('a CARGO TARGET binary is the one to copy, and a PACKAGED one is emphatically not', () => {
  ...
  assert.ok(isCargoTargetBinary('C:/repo/engine/target/debug/zengine.exe'))
  assert.ok(isCargoTargetBinary('C:/repo/engine/target/release/zengine.exe'))
  ...
  assert.equal(isCargoTargetBinary('C:/app/resources/engine/zengine.exe'), false)
  assert.equal(isCargoTargetBinary('C:/app/resources/zengine.exe'), false)
  ...
  assert.equal(isCargoTargetBinary('C:/app/target/debug/zengine.exe'), false)
  assert.equal(isCargoTargetBinary('C:/Users/x/AppData/Roaming/eqc/engine-run/zengine.exe'), false)
})
...
test('the predicate is SEPARATOR-BLIND, because `node:path` hands back the other one', () => {
  ...
  assert.ok(isCargoTargetBinary('C:\\repo\\engine\\target\\debug\\zengine.exe'))
  assert.ok(isCargoTargetBinary('C:\\repo/engine\\target/release\\zengine.exe'))
})

test('the staged names keep the EXTENSION, because Windows will not execute a file without one', () => {
  const names = stagedEngineNames('zengine.exe')
  assert.deepEqual(names, ['zengine.exe', 'zengine-1.exe', 'zengine-2.exe', 'zengine-3.exe'])
  ...
  assert.equal(names[0], 'zengine.exe')
...
test('a name with no extension still gets distinct siblings (the non-Windows spelling)', () => {
  assert.deepEqual(stagedEngineNames('zengine'), ['zengine', 'zengine-1', 'zengine-2', 'zengine-3'])
