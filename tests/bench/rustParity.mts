 ...
 *
 *   Z ENGINE 2026-09-08 — the parser now claims three sentences that were `unknown` and CAPTURES
 *   one clause it used to discard. Phase 1: the byte-identity bar reds on FOUR shapes and no
 *   others.
 *     • `You say, 'Hail[,] <NPC>'` → `hail` where the golden says `unknown`. ZERO lines in the
 *       committed fixture corpus (`tests/fixtures/*.log` holds no chat at all), so this shape can
 *       only red on a recorded SLICE, which is the owner's real log.
 *     • `Your faction standing with <F> has been adjusted by <n>.` → `factionHit` where the golden
 *       says `unknown` (97 fixture lines).
 *     • `… could not possibly get any worse.` / `… any better.` → `factionHit` carrying `cap`
 *       rather than `delta` (130 + 16 fixture lines). The saturation forms state NO magnitude and
 *       are counted, never summed — a `delta` on one of these would be drift, not this change.
 *     • THE ONE CHANGE TO AN EXISTING SERIALIZED EVENT: a `loot` event with
 *       `disposition:"sold"` now carries a trailing `coins` object. 1,219 fixture lines — 981 with
 *       a price, 238 `for free.` which write the present-and-empty `{}` (the shape
 *       `purchase.price` has used for the free merchant form since JOS-144); 564 of the 1,219 are
 *       in `wl44-swap-boundary.log` alone. The key is APPENDED after every field a sold row already
 *       carried, so the bytes AHEAD of `"coins":` are byte-identical to the golden's — a sold-loot
 *       divergence anywhere but that trailing key is still drift.
 *   Phase 2 is unaffected on every module the goldens carry: the five modules that read these
 *   (`hails`, `faction`, `coin`, `skills`, `deaths`) are NOT in the recorded TypeScript, so they are
 *   reported as missing rather than compared, and `loot`'s own rows gained no field. `skillUp` was
