...
// WRAPPED SEVERAL NAMES TO A LINE, NOT ONE PER LINE, and that is a factoring decision rather than
// a style one: this file is AT the measured 400-code-line ceiling, a type-only import line still
// costs a code line, and thirty-five of them were a third of the budget spent on a list nobody
// reads down. The names are unchanged and still alphabetical.
import type {
  AlertDef, AlertPrefs, AppFocus, CharacterRef, EqConfig, EqConfigResult, FeedReport, FiredAlert,
  ItemKnowledge, LogLine, LogSwitchNudge, LootEvent, MobKnowledge, ModuleChanged, ModuleSnapshot,
  PackInstallProgress, PackMutationResult, PackPreviewList, ProgressState, RegistryListResult,
  SoundData, SoundPack, SpeechEngine, SpeechInstallProgress, SpeechInstallResult, SpeechSayRequest,
  SpeechSayResult, SpeechVoice, SpellCatalog, UpdateStatus, UserSound, UserSoundImportResult,
  UserSoundRemoveResult, VoicePrefs
} from '../shared/types'
...
const api = {
  ...
  // ---- the quest tracker's slice (src/preload/questsApi.ts) ----
  ...questsApi,
  // ---- the Crafting tab's recipe index (src/preload/craftApi.ts) ----
  ...craftApi,
