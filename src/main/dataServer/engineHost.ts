...
//     `resources/engine/zengine.exe` beside the asar, the first path `engineBinaryCandidates`
 ...
 * the dev's next move obvious (`cargo build --release -p zengine` — RELEASE since JOS-520, which is
...
function resolveEngineBinary(): string | null {
  ...
  if (found === undefined) {
    ...
    // built ONLY `cargo build -p zengine` now resolves nothing, because `target/debug` is no longer
    ...
    if (!app.isPackaged && env.profile === undefined) {
      logInfo(
        `[everquest-companion] …a cargo DEBUG build is not a candidate unless a launch names it: ` +
          `set ${ENGINE_PROFILE_ENV}=debug to run it, or build release with \`cargo build --release -p zengine\``
      )
    }
    return null
  }
...
// that spawns `engine/target/debug/zengine.exe` makes `cargo build -p zengine` fail at the LINK
...
//     against cargo's own output directories, so a shipped `resources/engine/zengine.exe` is spawned
...
//     arrangement, restated: `extraResources` puts `zengine.exe` alone under `resources/engine/`
