...
Electron main's whole relationship with the Rust engine (`engine/crates/zengine`, JOS-459). The
design and the owner's twenty rulings live in `docs/plans/data-server.md`; the engine's own side is
`engine/crates/zengine/README.md`. Every file here carries its argument in its header — this page is
...
`resources/engine/zengine.exe`), so default-on there means the engine actually runs. Since
...
while the panel is open** — see "The polling discipline" in `engine/crates/zengine/README.md` for the
...
```
 startEngineSupervisor()          [engineHost.ts — unconditional since JOS-499]
   ...
   └─ supervisor.start()
        spawn zengine.exe ─── token down stdin ──►  engine
...
1. **The engine parses `spells_us.txt` INTERNALLY, for its own joins.** The path derives from the
   ...
   `zengine/src/concard.rs` currently sends empty and says so at length.
...
* Item 4 was **not built** in JOS-496 — only measured and ruled. **JOS-497 item 3 built the engine
  ...
  172 and not 173), `zengine::spells` is the file (path derived from the attach's log grandparent,
...
**The incident.** `cargo test` writes `engine/target/debug/zengine.exe` as a side effect of running
...
**Untouched:** the packaged candidates (`resources/engine/zengine.exe` first — `tests/
