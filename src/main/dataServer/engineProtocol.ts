 ...
 *
 * With a stand-in binary in place of `zengine.exe`, the child read the token off stdin, failed to
...
// copies the RELEASE binary to `resources/engine/zengine.exe` — the address the packaged candidate
...
// test` writes `target/debug/zengine.exe` as a side effect of running the engine's own unit tests,
...
export const ENGINE_BIN_NAME = process.platform === 'win32' ? 'zengine.exe' : 'zengine'
...
export interface EngineBinaryEnv {
   ...
   * candidate only, so a `target/debug/zengine.exe` that `cargo test` left behind can never be
 ...
 * process. So a dev app that spawns `engine/target/debug/zengine.exe` directly holds that exact path
 * open for as long as the app is up — and the next `cargo build -p zengine` fails at the LINK step
 ...
 * the only writer that matters. A packaged build's `resources/engine/zengine.exe` has no compiler
