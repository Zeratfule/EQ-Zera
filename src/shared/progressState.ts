...
import type { QuestPin } from './questPins'
...
export interface ProgressState {
  ...
  wishlist?: WishList
  /**
   * THE QUEST TRACKER (EQ Zera) — the catalog quests this character is following, each with the
   * checklist steps the player ticked BY HAND and the instant a completion was recorded
   * (`shared/questPins.ts`). Character-scoped for the reason every key here is: a quest is run by
   * one character.
   *
   * ONLY THE HAND STATEMENTS ARE IN IT. Steps the LOG satisfies are re-derived on every read from
   * the loot history and the turn-in ledger (`shared/questProgress.ts`) and are never written here
   * — the classUnlocks evidence rule again: observed evidence is recomputed and can change its
   * mind, a user's own statement is kept. Persisting a derived tick would forge a statement the
   * player never made and would survive the evidence being corrected.
   *
   * ADDITIVE and OPTIONAL — no schema bump and no migration, the `exaltPlans` precedent exactly:
   * every reader defaults on a missing key and electron-store rewrites the whole parsed object, so
   * a store written by any older build loads unchanged and one written here still opens in a build
   * that predates the tracker (`tests/questPins.test.mts` pins the sanitizer both ways).
   */
  questPins?: QuestPin[]
  /**
