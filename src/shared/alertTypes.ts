...
export type LogEventKind =
  ...
  | 'spellSet'
  | 'consider'
  // THE QUEST RUN (Z Engine, 2026-09-08). `hail` carries `{npc}` — the greeting that opens every
  // NPC dialogue tree, and the only `You say` line the parser claims. `factionHit` carries
  // `{faction, delta?, cap?}`, so `where:{cap:'min'}` is "this faction cannot get any worse" and
  // `where:{faction:'/guk/'}` is a whole ladder. The two fields are mutually exclusive: the
  // saturation lines state no magnitude at all.
  | 'hail'
  | 'factionHit'
  // A SKILL TICKED (`You have become better at Meditate! (57)`). Parsed since JOS-119 and gated out
  // of the editor by this list alone. `{skill, value?}` — `where:{skill:'Meditate'}` is one lane.
  | 'skillUp'
 ...
 *
 * The two WISH signals are the wish list speaking: `wishDrop` when a live loot line names an item
 * on the list (your OWN loot - the log corpus carries no "somebody else looted" line at all), and
 * `wishZone` when the character walks into a zone the mob catalog says a wished item drops in.
 * Both are fired by `features/wishlist/useWishAlerts.ts`, the one always-mounted place that can
 * see either.
 */
export type AppSignal = 'bossDefeat' | 'questComplete' | 'wishDrop' | 'wishZone'
