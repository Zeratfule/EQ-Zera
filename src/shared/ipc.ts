...
export const IPC = {
  ...
  // ---- the quest tracker (EQ Zera) ----
  // The active character's TRACKED QUESTS — the catalog quests they are following, each with the
  // checklist steps they ticked BY HAND and the instant a completion was recorded
  // (shared/questPins.ts). Read/write pair over `ProgressState.questPins`, exactly the wish list's
  // arrangement: the renderer is UNTRUSTED, so a written list is re-validated entry by entry
  // (`sanitizeQuestPins`) before a byte of it reaches the store, and the SAME validator runs on the
  // way out, so the round trip is a fixed point. WHOLE-DOCUMENT, because the renderer edits the
  // tracker as one small list (tens of rows at most) and a partial write would leave a pin and its
  // ticks disagreeing.
  //
  // A set PUSHES `onProgress` the way `setQuestTurnIns` does: a tick made on the quest page has to
  // reach the tracked-quests list and the completion watch without a refetch race.
  questPinsGet: 'questPins:get',
  questPinsSet: 'questPins:set',
  ...
  /** EQ Zera: a classic race model read out of the game's own global_chr.s3d (character-model spike). */
  eqModel: 'eqassets:model',
  /**
   * EQ Zera: the LOOK of an item by NAME (model actor, material code, dye) out of the committed item
   * table (`main/data/itemModels.json`), for previewing a wearable on the character model. The
   * table is keyed by item id and lives in main; this is the one name-keyed door into it, and it
   * answers `null` for a name it does not know rather than guessing (law 12). `looks` counts how
   * many distinct looks the name carries when the table's ids disagree.
   */
