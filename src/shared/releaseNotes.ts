...
export const RELEASE_NOTES: readonly ReleaseNote[] = [
  ...
  // behind share.eqzera.com went live 2026-09-09 (share-server/, Cloudflare Worker + KV).
  {
    ...
    entries: [
      {
        kind: 'fixed',
        text: 'The character model now fills the share card portrait. The picture covers the tile\x27s full height above the caption, so the body is as tall as the tile allows, and the crop no longer keeps a faint band above the head.'
      }
    ]
  },
  {
  ...
  },
  {
    ...
    entries: [
      {
        kind: 'changed',
        text: 'Share links carry the card at full resolution. It is sent as a JPEG under the link service\'s new 1 MB limit, so the page shows it big and sharp instead of shrunk to fit.'
      }
    ]
  },
  {
  ...
  // the app grew the surfaces that read them. One release, because they shipped as one program.
  {
    ...
    entries: [
      {
        kind: 'new',
        text: 'Upgrade finder. Zone Loot rows and item pages carry an upgrade chip when a drop beats what you wear under the Build tab profile, Zone Loot can show upgrades only, and the Build tab lists where to farm your upgrades, zone by zone with the mobs that drop them.'
      },
      {
        kind: 'new',
        text: 'Quest tracker. Track any quest from its page and follow its steps as a checklist. Looting an item lights its step, a turn-in to the quest giver ticks the hand-in steps, hailing the NPC ticks the hail step, and turning in every required item completes the quest with a card and your quest-complete alert.'
      },
      {
        kind: 'new',
        text: 'A Crafting tab in the gear area: what you can make from what you hold, what you are one ingredient away from, and every recipe with what you have and what you are missing.'
      },
      {
        kind: 'new',
        text: 'Wish list alerts. Looting a wished item and entering a zone where wished items drop each fire an alert you can tune and a card in the celebration overlay. Your own loot only: the log has no line for what other players loot.'
      },
      {
        kind: 'new',
        text: 'Death recap. When you die, a card says who killed you and what hit you in the last fifteen seconds, and the Overview keeps your last death with every hit, attacker and resist. Never for deaths already in the log.'
      },
      {
        kind: 'new',
        text: 'A Farm meter overlay: kills, XP, drops and coin per hour since you zoned in, plus your next watched respawn. The Character tab gains faction standing and skill-up panels, the Leveling tab a coin per hour tile, and the Overview a last session summary.'
      }
    ]
  },
  {
    ...
    entries: [
      {
        kind: 'new',
        text: 'Quest item pop-ups. Loot an item a quest needs or hands out and a card appears in the celebration overlay naming the quest, who gives it and where, with its steps listed and the step that needs the item lit. Click the quest to open its page on the Quests tab.'
      },
      {
        kind: 'new',
        text: 'Quest item pop-ups have their own switch under Preferences - Overlays, with a filter for quests your classes can take and a choice of how long the same item waits before it pops up again.'
      },
      {
        kind: 'new',
        text: 'A Zone Loot tab. Pick any zone and see every mob the wiki lists there and what each drops, with how many you have looted yourself and your drops per hour in that zone beside the wiki list, and out-of-era drops folded behind one chip.'
      },
      {
        kind: 'new',
        text: 'Preview any wearable item on your character model. The Gear tab and every item page carry a Preview on me button that tries the item in its slot on the Character tab, with one click back to your real gear. Armour shows as its material and dye and held items as their own model; slots the game draws nothing for say so.'
      }
    ]
  },
  {
