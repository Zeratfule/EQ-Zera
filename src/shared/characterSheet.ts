...
export interface SheetItem {
  ...
  exaltations: string[]
  /**
   * THE ORNAMENTATION (EQ Zera, 2026-09-06): the donor item whose LOOK this item wears, read off
   * the `-Slot2` socket (inventorySlots.ts measured the socket numbering: 2 is the cosmetic one).
   * It is in `exaltations` too, like every socket; this names it separately because the character
   * model draws it and the stat totals must not.
   */
  ornament?: string
  /** the ornament donor's item id, as the dump wrote it - the key into the game's item table */
  ornamentId?: number
}

/** The socket index the client keeps an ornamentation in - see `FOCUS_SOCKET_INDEX` for the measurement. */
export const ORNAMENT_SOCKET_INDEX = 2
...
function sheetItem(entry: InventoryEntry): SheetItem {
  ...
  // The client spells the cosmetic socket's copy `(Ornamentation)`; the socket index is the
  // fallback for a dump that spells every socket `(Exaltation)`.
  const ornament = entry.children.find(
    (c) => !c.empty && (c.parsedName.ornamentation || (c.parsedName.exaltation && c.path[c.path.length - 1] === ORNAMENT_SOCKET_INDEX))
  )
  if (ornament) {
    item.ornament = ornament.parsedName.base
    item.ornamentId = ornament.itemId
  }
  return item
}
...
export interface SheetItemView extends SheetItem {
  ...
  skill?: string
  /** the game's own model for this item (`IT68`), from the item table keyed by the dump's item id */
  model?: string
  /** the client's armour material code (0 cloth, 1 leather, 2 chain, 3 plate), from the item table */
  material?: number
  /** the dye colour as the client stores it (ARGB), from the item table; 0 = none */
  color?: number
  /** the ornament donor's own model, material and dye - what the model draws when ornaments are shown */
  ornamentModel?: string
  ornamentMaterial?: number
