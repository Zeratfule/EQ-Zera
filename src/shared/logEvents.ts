...
// The three statements the SERVER makes about what you just did (Z Engine, 2026-09-08). Each lives
// beside its own module's snapshot shape, the ./acquireEvents split's argument applied again: this
// file is at its measured 400-code-line ceiling, so a new shape goes where its surface is already
// documented. Re-exported verbatim, so no importer moved.
...
export interface LootEventE extends LogEventBase {
  ...
  created?: string
  /**
   * WHAT THE AUTO-VENDOR PAID, on the 'sold' disposition and nowhere else.
   *
   * The sentence has always stated it (`… and sold it for 60 platinum, 7 gold, 1 silver and 4
   * copper.`); the parser's money clause was non-capturing until 2026-09-08, so the price was
   * read and discarded on every one of them. It is the largest coin stream a farming session
   * produces, which is why plat-per-hour was unanswerable before.
   *
   * Present-and-EMPTY for `… and sold it for free.` — the line DID state a price and the price was
   * nothing, the same claim `PurchaseEvent.price` makes for the free merchant form. Absent when
   * the clause could not be read at all, and absent on every other disposition.
   *
   * NOT SUMMED, HERE OR ANYWHERE: see the COIN IS NOT SUMMED HERE law in ./acquireEvents.
   */
  coins?: Coins
}
...
export type LogEvent =
  ...
  | SelfWhoEvent
  | SkillUpEvent
  // The quest run's two sentences (Z Engine, 2026-09-08). Both were MEASURED `{kind:'unknown'}`:
  // `You say, 'Hail…` because no chat line was ever claimed, and the faction family because
  // nothing read it. They sit here rather than beside loot because both are statements the SERVER
  // makes about what you just did, which is what `selfWho` and `skillUp` are.
  | HailEvent
  | FactionHitEvent
  | SpecialAttackEvent
