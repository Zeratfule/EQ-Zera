...
export const GROUP_PACK_ID = 'eq-zera-console'
...
/**
 * The console pack's cues used by the groups (ids from scripts/gen-sounds.mts; the category slug
 * leads each id so the fallback resolution keeps the INTENT). What each cue IS, so a future edit
 * keeps the intent rather than the id.
 */
const SOUND = {
  /** the low thud — you cannot reach what you are attacking. */
  outOfRange: 'resource-limit-thud-01',
  /** the buzz — your spell was shrugged off. */
  resisted: 'task-error-buzz-01',
  /** the two-note "wrong" — the cast never happened. */
  castFailed: 'task-error-buzz-02',
  /** the falling drop — you are becoming visible again. */
  invisDrop: 'resource-limit-thud-02',
  /** the low two-note — the mana is not there. */
  outOfMana: 'resource-limit-thud-03',
  /** the slip — the spell bounced off an existing effect. */
  spellBlocked: 'task-error-buzz-03',
  /** the slip again, flat — you are dead. */
  playerDeath: 'task-error-buzz-03',
  /** the alarm — the seeded charm-break cue. */
  charmBreak: 'input-required-alarm-01',
  /** the siren — the thing you were holding is loose again. */
  ccBreak: 'input-required-alarm-02',
  /** the blip — a debuff took hold on the mob in front of you. */
  debuffLanded: 'task-acknowledge-blip-01',
  /** the triple warning — the slow expired; it needs re-landing. */
  slowExpired: 'input-required-alarm-03',
  /** the ping — somebody is waiting on a reply. A seeded def may only point at the bundled pack. */
  tellReceived: 'task-acknowledge-blip-02',
  /** the cleared fanfare — something worth stopping for hit your bags. */
  moteLooted: 'task-complete-fanfare-03'
} as const
