 ...
 * `<Item> (Exaltation)` spelling for a socketed copy of an item - and its `<Item> (Ornamentation)`
 * spelling for the cosmetic socket (EQ Zera, 2026-09-06: the live dump writes that on `-Slot2`
 * rows), which sets `ornamentation` as well, so every socket-copy rule applies to both and only
 * the character model tells them apart; `starred` records a trailing `*` whose meaning the file
 * never states.
 */
export interface ParsedItemName {
  ...
  tier?: number
  exaltation: boolean
  /** the socket copy is the cosmetic one: `<Item> (Ornamentation)` */
  ornamentation: boolean
  starred: boolean
}

const EXALTATION_SUFFIX = ' (Exaltation)'
const ORNAMENTATION_SUFFIX = ' (Ornamentation)'

export function parseItemName(name: string): ParsedItemName {
  let base = name
  const ornamentation = base.endsWith(ORNAMENTATION_SUFFIX)
  const exaltation = ornamentation || base.endsWith(EXALTATION_SUFFIX)
  if (ornamentation) base = base.slice(0, -ORNAMENTATION_SUFFIX.length)
  else if (exaltation) base = base.slice(0, -EXALTATION_SUFFIX.length)
