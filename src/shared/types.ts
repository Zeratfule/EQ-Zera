 ...
 *
 * ── THE TENTH ('farm', roadmap 2 item 8) IS THE ONE THAT IS NOT AT THE END, AND THAT IS EXACT ──
 *
 * The FARM METER: kills, XP, drops and coin per hour since you zoned in, over the current zone
 * stay. A placeable window like the nine above (its own bounds, lock, alpha and text size), MUI-
 * free, `chrome` hot zones, default OFF; it reads `progression`, `loot`, `coin`, `respawn` and
 * `character`, so it is in `worldRebuilt.MODULE_READING_OVERLAYS` too. `renderer/overlay/farmRows.ts`
 * carries the arithmetic and the DECLARED coin ladder that goes with it.
 *
 * IT SITS BEFORE THE THREE STRIPS RATHER THAN AT THE END, and the rule above is not weakened by
 * that — read what the rule is FOR. A dock slot is `METER_KINDS.indexOf(kind)` (overlayLayout.ts),
 * and `METER_KINDS` is this list with the strips filtered OUT. So inserting a METER anywhere among
 * the strips is append-only where it counts: every existing meter keeps its index, and 'farm' takes
 * the next free slot. What the rule actually forbids — moving somebody's window — cannot happen
 * here. Two things make the position load-bearing in the other direction:
 *   · tests/conCard.test.mts pins `conCard` as the LAST member of this list, so a kind appended
 *     after it is a red build stating that the strips are no longer the tail;
 *   · shared/overlayLabels.ts groups the same way ("the windows you place, then the strips") and
 *     tests/overlayLabels.test.mts pins `OVERLAY_LABEL_ORDER.slice(-3)` to the strip kinds.
 * MEASURED with the tenth in place (tests/overlayLayout.test.mts): 1080p and 1440p still seat every
...
export interface QuestEntry {
  ...
  expReward?: boolean
  /**
   * The quest's steps as plain text, one per line, from the wiki's Quick List / Checklist /
   * Short Walkthrough when the page has one, else its Walkthrough with the NPC dialogue,
   * faction lines and exp markers stripped (scripts/sources/questPage.ts `extractSteps`).
   */
  steps?: string[]
}
...
// The five surfaces added 2026-09-08. Each lives beside its OWN argument, on the characterTypes.ts
// precedent — this file is at its measured 400-code-line ceiling and takes re-exports only. Every
// one publishes a whole snapshot as its delta (the RespawnDelta posture; hailTypes.ts says why).
//
// ONE LINE EACH, AND THE TRANSPORT PAIR ONLY. The row types, the event shapes and the EMPTY_*
// defaults are exported from the surface files and imported from there — this file's budget is what
// stopped the loot/turnins habit of restating a whole module's vocabulary here, and re-exporting
// five more surfaces in full would have spent it twice over.
//
// hails: the NPCs you greeted, in order — the quest tracker's spine.
// faction: how far each faction moved, and how often it could not move at all. The saturation lines
//   are COUNTED, never summed; factionTypes.ts states that law in full.
// coin: every coin the log said reached you, one row per sentence, and NO TOTAL — the platinum
//   ladder is not in the log, so a consumer declares its own rate in the open.
// skills: what the client last SAID each skill is worth, beside the ticks this fold watched.
// deaths: the last seconds of incoming damage, by attacker, per death.
export type { HailDelta, HailSnap } from './hailTypes'
export type { FactionDelta, FactionSnap } from './factionTypes'
export type { CoinDelta, CoinSnap } from './coinTypes'
export type { SkillDelta, SkillSnap } from './skillTypes'
export type { DeathDelta, DeathRecap, DeathSnap } from './deathTypes'
...
export interface AppFocus {
  ...
  mob?: string
  /**
   * The quest to land on. For 'posky' the canonical `Class::Name` Sky quest key (expand + scroll);
   * for 'quests' the catalog quest's wiki PAGE title (`QuestEntry.page`), which opens its QuestPage.
   */
  quest?: string
