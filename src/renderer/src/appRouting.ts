...
export interface AppRouting {
  ...
  /** The catalog quest the Quests tab should open its page on, as a wiki PAGE title, or null. */
  questPage: string | null
  questPageNonce: number
  /**
   * Bare ⇒ the Quests tab as it stands. With a page ⇒ the quest-item toast's deep link: the same
   * tab, opened on that quest's page (ROADMAP §1). ONE opener, like `openLoot` and `openLeveling`:
   * one destination differing only in whether a payload rode along. A page the committed catalog
   * does not know simply lands on the list — the anchor is a request, never a promise.
   */
  openQuestPage: (page?: string) => void
  clearQuestPageFocus: () => void
  ...
  /** The item the Character tab should PREVIEW on the model, or null for the sheet as it stands. */
  previewItem: string | null
  previewNonce: number
  /**
   * Bare ⇒ the Character tab (the sheet, as the dump wrote it). With an item name ⇒ the DEEP link
   * this app's item pages carry: the same tab, with that item drawn on the model in its own slot
   * in place of what is worn there, and a banner naming it with the way back. ONE opener, exactly
   * like `openLoot` and `openLeveling`: one destination differing only in whether a payload rode
   * along - and the payload is a REQUEST, never a promise, because an item the model cannot wear
   * still lands here and is told so in one line.
   */
  openCharacter: (item?: string) => void
...
export function useAppRouting(view: View, setView: (v: View) => void): AppRouting {
  ...
  // The Quests tab's own anchor (ROADMAP §1) — a wiki PAGE title, which is what `QUEST_BY_PAGE`
  // resolves and what the quest-item toast's focus carries.
  const questPage = useFocusSlot<string>('quests', linkTo)
  ...
  // THE ITEM PREVIEW (EQ Zera): an item page asking the character model to wear its item. Same
  // shape as every other slot — the NONCE is what makes asking twice arrive twice, which matters
  // more here than anywhere else because the Character tab UNMOUNTS on every tab switch and a
  // second preview of the same item is otherwise indistinguishable from the first.
  const character = useFocusSlot<string>('character', linkTo)
  ...
  return useMemo(
    () => ({
      ...
      questPageNonce: questPage.nonce,
      openQuestPage: questPage.open,
      clearQuestPageFocus: questPage.clear,
      ...
      previewItem: character.value,
      previewNonce: character.nonce,
      openCharacter: character.open,
      clearPreviewFocus: character.clear
    }),
    [selectView, nav, spell, mob, quest, questPage, level, combat, loot, character]
  )
}
