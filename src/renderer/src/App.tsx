...
import { QuestsBranch } from './features/quests/QuestsView'
import { ZoneLootBranch } from './features/zoneloot/ZoneLootView'
import { CraftingBranch } from './features/crafting/CraftingView'
import { BuildBranch } from './features/build/BuildView'
    ...
    <>
      ...
      {view === 'loot' && (
        <LootView
          ...
          nav={routing.nav}
          onPreview={routing.openCharacter}
        />
      )}
      ...
      {view === 'gear' && <GearView key={viewKey} onOpenLoot={routing.openLoot} onPreview={routing.openCharacter} />}
      ...
      {/* …and it takes the item-preview focus like every other deep-link receiver: the payload
          arrives on the nonce, which this tab reads on MOUNT as well as on a change, because
          unlike Loot or Mobs it is remounted on every visit (the `key` above). */}
      {view === 'character' && (
        <CharacterView
          key={viewKey}
      ...
      {/* QUESTS (EQ Zera): mounted as a sibling, for the same reason SpellDrill is. */}
      <QuestsBranch
        view={view}
        viewKey={viewKey}
        onOpenLoot={routing.openLoot}
        anchor={routing.questPage}
        anchorNonce={routing.questPageNonce}
        onAnchorApplied={routing.clearQuestPageFocus}
      />
      {/* ZONE LOOT (EQ Zera, ROADMAP §2): the same sibling idiom. Mob names drill into the Mobs
          tab, item names into the Loot drill-down, both through the router. */}
      <ZoneLootBranch view={view} viewKey={viewKey} onOpenLoot={routing.openLoot} onOpenMob={routing.openMob} />
      <BuildBranch view={view} viewKey={viewKey} onOpenLoot={routing.openLoot} onOpenMob={routing.openMob} />
      {/* CRAFTING (EQ Zera, roadmap 2 item 5): the gear area's sixth tab, the same sibling idiom. */}
      <CraftingBranch view={view} viewKey={viewKey} onOpenLoot={routing.openLoot} />
    </>
  )
}
 ...
 * Five destinations: the Mobs tab, optionally drilled into a specific mob (a click on the events
 * overlay's con rows); the Plane of Sky tab, optionally ANCHORED at the quest that just completed
 * (docs/plans/celebration-toasts.md T6, finished in wave O2); the Leveling tab, optionally
 * anchored at the level that just dinged (docs/plans/levelup-whats-new.md §2); and the Quests tab,
 * optionally opened on the catalog quest a looted item belongs to (ROADMAP §1); and the Wish list
 * tab, which takes no anchor at all.
 *
