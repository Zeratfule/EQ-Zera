...
export interface GearViewProps {
  ...
  /**
   * SEE A ROW ON YOUR CHARACTER (EQ Zera) — App's `openCharacter`, one tab over in this same gear
   * area. Threaded exactly the way `onOpenLoot` is: the view knows nothing about it beyond handing
   * it to the table, and the per-row control decides for itself whether the item is wearable.
   */
  onPreview?: (item: string) => void
}

export default function GearView({ onOpenLoot, onPreview }: GearViewProps = {}): JSX.Element {
    ...
    <Box sx={{ display: 'flex', flexDirection: 'column', height: '100%', minHeight: 0 }} data-testid="gear-view">
      ...
      >
        <GearTable
          ...
          // `onPreview` is EQ Zera's per-row "see it on me" gesture, hidden on any row the model
          // has no cell for. It shares a line with its neighbour because this function is AT the
          // measured lines-per-function ceiling and a per-prop line would have been a ratchet.
          onOpenLoot={onOpenLoot} onPreview={onPreview}
