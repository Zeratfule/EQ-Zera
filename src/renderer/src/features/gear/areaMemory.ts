...
import { BUILD_PROFILES, type BuildProfile } from '../../../../shared/build/profiles'
...
export const AREA_FORM_TIER = {
  ...
  // ---- the Build tab (EQ Zera) ----
  /** tank / dps / heal — the profile the meters and the upgrade list are read under */
  'eq.build.profile': 'restart',
  // ---- the Character tab ----
...
/** The Build tab's profile (EQ Zera): one of the three, or Tank — a closed vocabulary, no exemptions. */
export function sanitizeBuildProfile(raw: unknown): BuildProfile {
  return typeof raw === 'string' && (BUILD_PROFILES as readonly string[]).includes(raw) ? (raw as BuildProfile) : 'tank'
}
