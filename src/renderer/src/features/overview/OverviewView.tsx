...
//
// A FOURTH REGION JOINED THEM (ROADMAP items 7 and 9): the RECKONING band — last session, and last
// death. It is a fourth region rather than two more cards in the NOW rank because neither answers
// "what is happening right now", and that rank's row floor was measured for the three that do.
//
  ...
  return (
    <Stack spacing={1.5} sx={{ height: '100%', minHeight: 0, overflow: 'hidden' }}>
      ...
      >
        ...
        {/* THE RECKONING BAND (ROADMAP items 9 and 7). Two more twins, in tracks of their own
            beneath the feeds and NOT inside the NOW rank: neither describes right now, and adding
            either to that rank would have squeezed the three cards it was measured for. Both read
            their own modules, so this view still owns no state on their behalf, and both are
            internally bounded — the last-session card is a tile row, the death card's hit list
            lives in a fixed-height scroller of its own. */}
        <Box
          sx={{
            display: 'grid',
            gap: 1.5,
