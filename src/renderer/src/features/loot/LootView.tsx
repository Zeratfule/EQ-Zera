...
export interface LootViewProps {
  ...
  nav?: NavBack
  /** EQ Zera: show a looted item on the character model (App's `openCharacter`). Threaded down to
   *  the detail pane, which draws the control only for an item the model can actually wear. */
...
function LootDetailTakeover(p: TakeoverProps): JSX.Element {
  const { item, events, slice, detail, nav, invByKey, onPreview } = p
...
export default function LootView(props: LootViewProps = {}): JSX.Element {
  ...
  if (selected !== null) {
    const p = { item: selected, events: sliced, slice, detail, nav: props.nav, invByKey, onPreview: props.onPreview }
