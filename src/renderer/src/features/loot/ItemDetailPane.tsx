...
// EQ Zera: "put this on my character". It draws itself only when the gear index gives this item an
// equip slot the model has a cell for, so the great majority of item pages gain no control at all.
import PreviewButton from '../character/PreviewButton'
// EQ Zera: "would this be better than what I have on?" — the Build tab's own comparison, drawn
// under the breadcrumb and ABSENT when the answer is no or when there is nothing to compare with.
...
export interface ItemDetailPaneProps extends ItemDetailProps {
  ...
  onList: () => void
  /** Show this item on the character model (App's `openCharacter`). Absent ⇒ no preview control,
   *  the same absent-never-disabled rule every other optional door in this app follows. */
...
export function ItemDetailPane(props: ItemDetailPaneProps): JSX.Element {
  const { item, events, stats, isQuestItem, onBack, onList, onPreview, origin, slice, owned } = props
  ...
  return (
    <Stack spacing={1.5} data-testid="loot-detail" sx={{ height: '100%', minHeight: 0 }}>
      <Stack direction="row" spacing={1} alignItems="center" sx={{ minWidth: 0, flexShrink: 0 }}>
        ...
        <UpgradeLine item={item} />
        <PreviewButton item={item} onPreview={onPreview} testId="loot-preview" />
      </Stack>
