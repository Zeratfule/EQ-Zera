...
/**
 * THE PREVIEW TAG. The grid IS the sheet - what it says is what the dump said - so a cell showing
 * a previewed item has to SAY it is showing one rather than quietly swapping the name for an item
 * this character may not even own. One word, no hover, no explanation: the banner above the model
 * carries the sentence and the way back.
 */
function PreviewTag(): JSX.Element {
  return (
    <Chip
      ...
      size="small"
      variant="outlined"
      sx={{ height: 15, borderColor: PALETTE.accent, color: PALETTE.accent, '& .MuiChip-label': { px: 0.5, fontSize: 9, lineHeight: 1.6 } }}
    />
  )
}

/** One cell: icon, slot label, the item name (hoverable) with its exaltations, or a quiet empty line. */
function SlotCell({ cell, previewing }: { cell: SheetCellView; previewing: boolean }): JSX.Element {
  ...
  return (
    <Paper
      ...
      sx={{
        ...
        alignItems: 'center',
        minWidth: 0,
        ...(previewing ? { borderColor: PALETTE.accent, borderWidth: 2 } : {})
      }}
    >
      <SlotIcon cell={cell} />
...
function Column({ cells, previewSlot }: { cells: SheetCellView[]; previewSlot?: string }): JSX.Element {
  return (
    <Stack spacing={0.6} sx={{ flex: 1, minWidth: 190 }}>
      {cells.map((c) => (
        <SlotCell key={c.id} cell={c} previewing={c.id === previewSlot} />
      ))}
    </Stack>
  )
}

const inColumn = (cells: readonly SheetCellView[], column: SheetColumn): SheetCellView[] =>
...
export default function SlotGrid({
  ...
  /** the model slot a preview has taken over, so that ONE cell can say it is not the dump's word */
  previewSlot?: string
}): JSX.Element {
  ...
  return (
    <Stack spacing={0.6} data-testid="character-slot-grid">
      <Stack direction={{ xs: 'column', md: 'row' }} spacing={0.6} alignItems="stretch">
        <Column cells={inColumn(cells, 'left')} previewSlot={previewSlot} />
        <Column cells={inColumn(cells, 'right')} previewSlot={previewSlot} />
      </Stack>
      ...
      <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.6 }}>
        {bottom.map((c) => (
          <Box key={c.id} sx={{ flex: '1 1 190px', minWidth: 190 }}>
            <SlotCell cell={c} previewing={c.id === previewSlot} />
          </Box>
        ))}
      </Box>
    </Stack>
