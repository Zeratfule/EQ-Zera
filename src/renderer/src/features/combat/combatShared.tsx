...
import { CATEGORY_LABEL, type DamageCategory } from '@shared/combat'
...
export const KIND_COLOR: Record<string, string> = {
  you: PALETTE.accent, pet: PALETTE.cyan, allyPet: PALETTE.cyanDim, member: '#7fbf8f', other: '#5f8f74', enemy: PALETTE.red
}
 ...
 *
 * `slay` used to be #e8d48a: a pale gold that was indistinguishable from the old melee gold at the
 ...
 * from the melee accent at any stripe width while staying high-contrast on the dark bg.
 */
export const CAT_COLOR: Record<DamageCategory, string> = {
  melee: PALETTE.accent,
  slay: '#f6f0da',
  spell: PALETTE.amber,
  dot: PALETTE.cyan,
  ds: PALETTE.red
  ...
  return (
    ...
    >
      <Stack
        ...
        alignItems="baseline"
        spacing={1}
        className="eq-card-rule"
        sx={{ mb: 0.75, pb: 0.5, flexShrink: 0 }}
      >
        {/* Both classes live in theme.ts (CssBaseline) — this file is at the max-lines ceiling. */}
        <Typography variant="caption" noWrap className="eq-display-label" sx={{ color: 'text.secondary' }}>
          {title}
        </Typography>
        {right}
      </Stack>
