...
export const MARKER_COLOR: Record<TimelineMarker['kind'], string> = {
  stance: PALETTE.accent,
  invocation: PALETTE.amber,
  coat: '#c46fd2',
  slow: '#57e0a0'
}
 ...
 */
export const CLICK_COLOR = PALETTE.cyan
...
export const ORIGIN_COLOR: Record<ProcOrigin, string> = {
  ...
  spell: PALETTE.amber,
  slay: '#f6f0da',
  // THE SWING-BORNE AA hue (JOS-437) — the stance accent, which is the one colour in the marker
  // vocabulary no proc origin had taken. `slay` and `aa` are the two lanes that ride an ordinary
  // weapon swing; cream is a near-white and this is the violet accent, so they stay
  // distinguishable at any stripe width.
  aa: PALETTE.accent,
  click: CLICK_COLOR
}
